...
<div class="max-w-6xl mx-auto space-y-6">
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 flex items-center justify-between gap-4">
        <div>
            <h1 class="text-3xl font-bold text-gray-900">Trash - Products</h1>
            <p class="text-gray-500 mt-1">Manage deleted products</p>
        </div>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        <div class="px-6 py-4 border-b border-gray-100 bg-gray-50 flex items-center justify-between">
            <p class="font-semibold text-gray-900">
                Deleted Products ({{ $trashedProducts->total() }})
            </p>
            ...
            <table class="w-full  min-w-[1100px]">
                <thead class="bg-white text-xs uppercase text-gray-500 border-b border-gray-100">
                    <tr>
                        ...
                        <th class="px-6 py-4 w-28 text-left">Image</th>
                        <th class="px-6 py-4 w-56 text-left">Name</th>
                        <th class="px-6 py-4 w-56 text-left">Category</th>
                    ...
                    @forelse($trashedProducts as $product)
                        ...
                        <tr class="hover:bg-purple-50/40">
                            <td class="px-6 py-4 text-gray-700">
                                {{ ($trashedProducts->firstItem() ?? 0) + $loop->index }}
                            </td>

                            <td class="px-6 py-4">
                                <div class="w-14 h-14 rounded-xl overflow-hidden bg-gray-100 border border-gray-200 shadow-sm">
                                    <img src="{{ $img }}"
                                         alt="Product image"
                            ...
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-3 min-w-0">
                                    ...
                                    <div class="min-w-0">
                                        ...
                                        <p class="text-xs text-gray-500 truncate">ID: #{{ $product->id }}</p>
                                    </div>
                                </div>
                            </td>
                                ...
                                <div class="flex justify-end gap-2">
                                    @can('restore', $product)
                                        <form method="POST"
                                            ...
                                            @csrf
                                            <button type="button"
                                                    class="restore-btn px-4 py-2 rounded-xl font-semibold border transition flex items-center gap-2 bg-purple-50 text-purple-700 border-purple-200 hover:bg-purple-100"
                                    ...
                                    @can('forceDelete', $product)
                                        <form method="POST"
                                            ...
                                            @method('DELETE')
                                            <button type="button"
                                                    class="force-delete-btn px-4 py-2 rounded-xl font-semibold border transition flex items-center gap-2 bg-pink-50 text-pink-700 border-pink-200 hover:bg-pink-100"
                                                    data-name="{{ $product->name }}">
                                    ...
                                    @if(!auth()->user()->can('restore', $product) && !auth()->user()->can('forceDelete', $product))
                        ...
                        <tr>
                            <td colspan="8" class="px-6 py-16 text-center text-gray-500">
                                <div class="flex flex-col items-center">
                                    ...
                                    <p class="text-sm text-gray-400 mt-1">No deleted products found</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
...
<div id="restoreModal" class="fixed inset-0 hidden z-50 flex items-center justify-center bg-black/50 p-4">
    <div class="bg-white rounded-2xl p-6 w-full max-w-md border border-gray-100 shadow-xl">
        ...
        <p class="text-xs text-gray-400 mt-1">This will move it back to active products.</p>
